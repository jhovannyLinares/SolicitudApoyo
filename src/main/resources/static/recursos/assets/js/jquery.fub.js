		...
		var validation = Array.prototype.filter.call(forms, function (form) {
			...
			form.addEventListener('submit', function (event) {
				...
				if (form.checkValidity() === false) {
					...
					if (document.getElementById('solIncorporacion') !=null){
...
function cargarMotivosCobro() {
	...
	case 1:
		...
		document.getElementById("si").classList.add('readonly');
		document.getElementById("no").disabled=true;
		document.getElementById("no").classList.add('readonly');
	...
	case 5:
		...
		document.getElementById("clabeCobro").removeAttribute('readonly');
		document.getElementById("clabeCobro").classList.remove('readonly');
		document.getElementById("si").removeAttribute('readonly');
		document.getElementById("si").classList.remove('readonly');
		document.getElementById("no").removeAttribute('readonly');
		document.getElementById("no").classList.remove('readonly');
...
function validFechaNac(input,campo) {
	...
	if (fechValida) {
		...
		if ((ed < 65 && idPrograma == 1) || ed > 130 || (ed < 18 && idPrograma == 3)) {
...
//funcion Monto Ninias y Nninios 

function validMontoNinios(input, disc,mont) {
	var monto = document.getElementById(mont);
	var disca = document.getElementById(disc).value;

	if (disca > 0) {
		monto.value="$ 3,600.00";
	}else{
		monto.value="$ 1,600.00";	
	}
//Funciona aqui
}

//Funcion valida ninios
function validaninios(input, id, disc, mont, hidden){
	validFechaNini(input,id);
	document.getElementById(hidden).value = ed;
	var discapacitado = document.getElementById(disc);
	var monto = document.getElementById(mont);
	var edad = ed;
	if(edad > 0 && edad < 4){
		input.setCustomValidity("");
		discapacidad.required=false;
		monto.value="$ 1,600.00";
...
//Funcion apoyo ninias y ninos 
function validFechaNini(input,campo,disc,mont) {
	var Fnac = input.value;
	var Sfecha = Fnac.toString();
	var desf = Sfecha.split('-');
	var Fconcatenada = desf[2] + "/" + desf[1] + "/" + desf[0];
	var monto = document.getElementById("monto").value;
	var idDisca = document.getElementById('discapacidad').value;
	console.log(Fconcatenada);
	...
	if (fechValida) {
		...
		document.getElementById("edadNB").value = ed;
	} else {
		input.setCustomValidity("Invalid field");
		console.log("No es una fecha valida");
...
//----------------------------------------------------------------------------------------
//Funcion escolaridad ninios
function validaninios(input, id, esc, mont, hidden){
	validFechaNini(input,id);
	document.getElementById(hidden).value = ed;
	var estudio = document.getElementById(esc);
	var monto = document.getElementById(mont);
	var edad = ed;
	if(edad > 0 && edad < 4){
		input.setCustomValidity("");
		estudio.required=false;
		monto.value="$ 1,600.00";
		console.log(monto.value);
	}else if(edad > 3 && edad < 7){
		input.setCustomValidity("");
...
function validaedadmonto(value){

    if (value== 0 || value<= 5 ) {
        var escolaridadhijos=document.getElementById("gradosescolaridad");
        var montohijos=document.getElementById("Monto");
        
        escolaridadhijos.required=false;
        escolaridadhijos.disabled=true;
        montohijos.value="$ 360.00";
    }else if (value>= 6 || value<= 23 ) {
        escolaridadhijos=document.getElementById("gradosescolaridad");
		montohijos=document.getElementById("Monto");	
		montohijos.value="0";
        escolaridadhijos.disabled=false;
        escolaridadhijos.required=true;
    }
}
...
//funcion seleccion
function seleccionactividad(value){

	var op=document.getElementById("actividadCaminar");
	var op2=document.getElementById("actividadHablar");
	var op3=document.getElementById("actividadOracion");
	var op4=document.getElementById("acticidadSocializa")
	var op5=document.getElementById("actividadObjtos");
	var op6=document.getElementById("actividadTomaLapiz");
...
function validFechaNacHijosJefasFam(input,campo) {
	...
	if (fechValida) {
		...
		//document.getElementById("edadNI").value = ed;
...
//validar la escolaridad porla edad
function validarescolaridad(value){
	//document.getElementById(campo).value = edad;
	//	document.getElementById("edadHijosJefas").value = edad;
	// edadreal=document.getElementById

		if ( value >= 0 || value < 6 ) {
			
			//document.getElementById("gradosescolaridad").disabled=true;
			document.getElementById("gradosescolaridad").required=false;
		}else if ( value>= 6 || value < 24 ) {
			document.getElementById("gradosescolaridad").disabled=false;
			document.getElementById("gradosescolaridad").required=true;
		}
}


// valida la fecha de nacimiento de los de los responsables
function validFechaNacResponsable(input,campo) {
	var Fnac = input.value;
	var Sfecha = Fnac.toString();
	var desf = Sfecha.split('-');
	var Fconcatenada = desf[2] + "/" + desf[1] + "/" + desf[0];
	...
	if (fechValida) {
		...
		var ed = edad(fech);
		document.getElementById(campo).value = ed;
		document.getElementById("edadResponsable").value = ed;
		//var idPrograma = document.getElementById('programa').value;
...
function hijos(input,campo) {
	...
	if (fechValida) {
		...
		var ed = edad(fech);
		document.getElementById(campo).value = ed;
		document.getElementById("edadHijos").value = ed;
...
//funcion de numero de beneficiarios
function numBeneficiario(input){
	if(input.value.length > 0){
		var ben = parseInt(input.value);
		if(ben == 0 || ben > 3){
			input.setCustomValidity("Invalid field");
			alert("Solo puedes Ingresar un máximo de 3 Beneficiarios");
			input.value = null;
		}else{
			input.setCustomValidity("");
		}
...
function habilitaCheckestudios(idChe, IdInput, tipo,check2) {
...
// document.getElementById('prev').style.visibility = 'visible';
	...
	if(index==1){
		document.getElementById('jefasprev').style.visibility = 'hidden';
    }
...
window.onload = function(){
	 ...
	 if (document.getElementById('fechaVisita') !=null){
		 document.getElementById('fechaVisita').value=ano+"-"+mes+"-"+dia;
	 }
	 
	 if (document.getElementById('fechaNacimientoN') !=null){
		 document.getElementById('fechaNacimientoN').value=ano+"-"+mes+"-"+dia;
	 }

	 if (document.getElementById('fechaA') !=null){
		document.getElementById('fechaA').value=ano+"-"+mes+"-"+dia;
	}

	if (document.getElementById('Fecha_Alta') !=null){
		document.getElementById('Fecha_Alta').value=ano+"-"+mes+"-"+dia;
	}

	if (document.getElementById('fechaNacimientoM') !=null){
		document.getElementById('fechaNacimientoM').value=ano+"-"+mes+"-"+dia;
	}
